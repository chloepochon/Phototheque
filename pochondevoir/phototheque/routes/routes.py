...
def upload():
    ...
    if request.method == 'POST' and 'photo' in request.files:
        ...
        chemin = request.form.get("chemin", None)
        fichier = request.form.get("fichier", None)

        # On décide d'appliquer des contraintes d'obligation et de caractères sur certains
        # champs de la notice
        ...
        if os.path.isfile("phototheque/static/img/"+str(fichier)):
            errors.append("Ce nom de fichier existe déja. Veuillez le télécharger sous un autre nom, et modifier le chemin en conséquence.")
        ...
        if not errors:
            ...
            # On redigige directement après le téléchargement vers la notice de la photographie en question
...
def edit_notice(photo_id):
    ...
    if request.method == 'POST':
        ...
        tag = request.form.get("tag", None)
        orientation = request.form.get("orientation", None)
        # Les champs fichier et chemin sont ici volontairement omis et ne sont utilisés qu'en cas
        # de remplacement

        # On décice d'appliquer des contraintes d'obligation et de caractères à certains champs de la notice
        if not titre or len(titre) == 0:
            errors.append("Titre obligatoire.")
...
def replace(photo_id):
    ...
    if request.method == 'POST' and 'photo' in request.files:
        ...
        if not errors:
            ...
            # ... et on supprime l'ancien fichier du dossier img pour ne pas créer d'interférence
            ...
            photos.save(request.files['photo'])
            flash("Votre photographie a bien été remplacée. Vous pouvez à présent si vous le souhaitez modifier sa notice.")
...
def delete(photo_id):
    ...
    # On récupère l'id du lien d'autorité crée à l'occasion du téléchargement initial de la photo :
    # On filtre pour obtenir l'id de l'enregistrement où l'id de la photographie concernée
    # dans la table Photo correspond à celui présent dans la colonne authorship_photo_id de la table Authorship
    authorship = Authorship.query.filter(photo_id == Authorship.authorship_photo_id).first()
    # On établit que cette jointure correspond à l'enregistrement de la table Authorship dont on veut l'id
    authorship_a_supprimer = authorship.authorship_id
    # On récupère cet id
    authorship_id= Authorship.query.get(authorship_a_supprimer)
    # Documentation qui m'a aidée à réaliser cette jointure :
    # https://flask-sqlalchemy.palletsprojects.com/en/2.x/queries/
    # https://qastack.fr/programming/27158573/how-to-delete-a-record-by-id-in-flask-sqlalchemy

    # Si la méthode est POST cela signifie que le formulaire est envoyé
    if request.method == 'POST':
        # https://askcodez.com/suppression-dune-ligne-avec-flask-sqlalchemy.html
        # On supprime l'enregistrement souhaité de la table Authorship
        db.session.delete(authorship_id)
    ...
    return render_template("pages/supprimer.html", photo=photo_a_supprimer, authorship=authorship, authorship_a_supprimer=authorship_a_supprimer, authorship_id=authorship_id, fichier_a_supprimer=fichier_a_supprimer)
