...
class User(UserMixin, db.Model):
    ...
    def identification(login, motdepasse):
        """ Fonction qui permet d'identifier un utilisateur.
         S'il n'y a pas d'derreur elle renvoie les données de l'utilisateur-ice.

        :param login: Nom d'utilisateur
        :param motdepasse: Mot de passe envoyé par l'utilisateur
        :returns: S'il n'y a pas d'erreur cela renvoie les données de l'utilisateur, sinon renvoie None
        :rtype: User or None
        """
        # On récupère un utilisateur unique dont le login renseigné correspond à celui référencé dans la table User
        utilisateur = User.query.filter(User.user_login == login).first()
        # Si le mot de passe rensigné concorde avec le nom d'uilisateur alors l'identification de
        # l'utilisateur-ice est réussie
    ...
    def creer(login, email, nom, motdepasse):
        """ Fonction qui crée un compte d'utilisateur-rice. Elle retourne un tuple (booléen, User ou liste).
        Si il y a une erreur, la fonction renvoie False suivi d'une liste d'erreur
        Sinon, elle renvoie True suivi de la donnée enregistrée

        :param login: Nom d'utilisateur de l'utilisateur-rice
        ...
        # On vérifie que personne n'a utilisé cet email ou ce nom d'utilisateur auparavant
        ...
        # On crée un utilisateur en faisant correspondre les champs de la table User avec les données
        # renseignées dans les champs
        utilisateur = User(
            user_nom=nom,
            user_login=login,
            user_email=email,
